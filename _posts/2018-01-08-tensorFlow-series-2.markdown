...
2. 我们定义一个代价函数（Cost function)，用于表示真实房价与我们计算房价差的绝对值，这里记为：abs(y-y’),这个代价函数的值越小，表示我们要求的方程越接近真实解。注意x/y都是我们手中资料库中已有的数据集，我们想用这些已有的数据集，来得出原来公式中a/b两个常量的值，从而完成方程求解，也就是“机器学习”的过程。
3. 我们随机为a/b取一个初始值。  
...
这张图描述了梯度下降法解方程的过程。需要说明的是，图中的J(thet0,thet1)就是代价函数，thet0、thet1代表两个要求解的系数值，请原谅这里不能键入希腊字母，导致这样的差异增加了阅读难度。图中手绘的部分，说明了我们一开始随机取了初始值，然后逐步的梯度下降，直到代价函数值最小的时候，得到两个未知数的解。  
