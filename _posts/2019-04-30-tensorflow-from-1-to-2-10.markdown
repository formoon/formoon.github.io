...
![](/attachments/201904/tensorFlow2/nmt-encdec.jpg)  
（图片来自谷歌NMT文档）  

注意力机制是人类特有的大脑思维方式，比如看到下面这幅照片：
![](/attachments/201904/tensorFlow2/nmt-attention.png)  
（图片来自互联网）  
...
![](/attachments/201904/tensorFlow2/nmt-seq2seq.jpg)  
（图片来自谷歌NMT文档）  

随后，我们为这个模型增加解码时候的权重机制。模型在处理每个单词输出的时候，会在权重的帮助下，把重点放在对应的输入单词上。示意图如下：  
![](/attachments/201904/tensorFlow2/nmt-greedy_dec.jpg)  
（图片来自谷歌NMT文档）  

最终，结合权重生成的过程，成为完整的注意力机制。注意力机制主要作用于解码，在每一个输出步骤中都要重新计算注意力权重，并更新到解码模型从而对输出产生影响。模型的示意图如下：  
![](/attachments/201904/tensorFlow2/nmt-attention_mechanism.jpg)  
（图片来自谷歌NMT文档）  
...
![](/attachments/201904/tensorFlow2/nmt-attention1.png)  
